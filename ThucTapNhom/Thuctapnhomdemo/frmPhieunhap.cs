...
namespace Thuctapnhomdemo
{
    public partial class frmPhieunhap : Form
    {
        ...
        private String connecttionString = @"Data Source=THAO-PC\SQLEXPRESS;Initial Catalog=QuanlyKhoHang;Integrated Security=True";
        ...
        private string SinhMaTuDong(string ma)
        {
            ...
            int chuoiSo = 0;
            string ChuoiMa = Convert.ToString(dgvPhieunhap.Rows[count - 2].Cells[0].Value);
        ...
        private void frmPhieunhap_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cbbMahanghoa.DataSource = LoadComBoBox("SELECT * FROM tbl_HangHoa ");
        ...
        public void trangthaidong(bool a)
        {
            txtTongtien.Enabled = a;
            txtDongianhap.Enabled = a;
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có muốn xóa không ?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                cmd = new SqlCommand("CTPhieuNhap_DELETE", conn);
                ...
                cmd = new SqlCommand("PhieuNhap_DELETE", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@sophieunhap", txtSophieunhap.Text.ToString());
                try
                {
        ...
        private void btnCapnhatct_Click(object sender, EventArgs e)
        {
            ...
            if (iSoluong > 0 && dDongianhap > 0)
            {
                ...
                for (int i = 0; i <= dgvData.Rows.Count - 2; i++)
                {
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (Trangthai == "ADD")
            {
                if (CheckTXT())
                {
                    ...
                    cmd = new SqlCommand("PhieuNhap_Them", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@sophieunhap", txtSophieunhap.Text.ToString());
                    ...
                    for (int i = 0; i < dgvData.Rows.Count - 1; i++)
                    {
                        ...
                        cmd = new SqlCommand("CTPhieuNhap_Them", conn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@sophieunhap", txtSophieunhap.Text.ToString());
            ...
            else if (Trangthai == "EDIT")
            {
                if (CheckTXT())
                {
                    ...
                    double dTongtien = 0;
                    double.TryParse(txtTongtien.Text.ToString(), out dTongtien);
                    cmd = new SqlCommand("PhieuNhap_EDIT", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@sophieunhap", txtSophieunhap.Text.ToString());
                    cmd.Parameters.AddWithValue("@ngaylapphieu", dtpNgaylapphieu.Value);
                    cmd.Parameters.AddWithValue("@nguoilapphieu", cbbNguoilapphieu.SelectedValue.ToString());
        ...
        private void dgvPhieunhap_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvPhieunhap.SelectedCells.Count > 0 && dgvPhieunhap.SelectedRows.Count > 0)
            {
                ...
                dtpNgaylapphieu.Text = dgvPhieunhap.Rows[Index].Cells[1].Value.ToString();
                string sCbbNguoilap = dgvPhieunhap.Rows[Index].Cells[2].Value.ToString();
                txtTongtien.Text = dgvPhieunhap.Rows[Index].Cells[3].Value.ToString();
                string sCbbNhomsp = dgvPhieunhap.Rows[Index].Cells[4].Value.ToString();
                string sCbbNCC = dgvPhieunhap.Rows[Index].Cells[5].Value.ToString();
